...
./check_bench_result.py output.txt
"""
...
# Allow script to run locally to check connection to Promethues without the
# need of a github token
try:
    ...
    print(
        f"WARNING: No github parameter {e} provided."
        "GITHUB reporting wil not work."
    )
...
if __name__ == "__main__":
    ...
    with open(results_file, "r") as file_handle:
        for result in file_handle:
            ...
            benchmark_last_value = benchmark_last_result(
                github_repo, benchmark_name
            )
            if benchmark_last_value == -1:
                print(
                    benchmark_name,
                    "is new and doesn't have any data yet, skipping",
                )
                continue
            if benchmark_current_value > benchmark_last_value:
                diff = difference_p(
                    benchmark_current_value, benchmark_last_value
                )
                if diff > threshold:
